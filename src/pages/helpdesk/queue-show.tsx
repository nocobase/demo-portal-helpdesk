...
export function QueueShow({ idParam = "id" }: { idParam?: string }) {
  ...
  const { result: queue, query } = useShow<NamedRecord>({ resource: "desk_queues", id: queueId });
  const { result: tickets, query: ticketsQuery } = useList<TicketRecord>({
    resource: "desk_tickets",
    filters: [{ field: "queue_id", operator: "eq", value: queueId ? Number(queueId) : undefined }],
    pagination: { mode: "server", currentPage: 1, pageSize: 100 },
    sorters: [{ field: "createdAt", order: "desc" }],
    queryOptions: { enabled: Boolean(queueId), retry: false },
  });

  return (
    <RouteDrawer
      title={query.isLoading && !queue ? translate("queues.show.title", { ns: "starter" }, "Queue") : (queue?.name ?? translate("queues.show.title", { ns: "starter" }, "Queue"))}
      description={translate("queues.show.description", { ns: "starter" }, "Tickets currently routed to this queue.")}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between rounded-xl border bg-muted/20 p-4">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                {translate("queues.show.ticketCount", { ns: "starter", count: tickets.data.length }, "{{count}} tickets")}
              </div>
            </div>
            <div className="divide-y overflow-hidden rounded-xl border">
              ...
              {!ticketsQuery.isLoading && tickets.data.length === 0 ? (
                <p className="p-8 text-center text-sm text-muted-foreground">
                  {translate("queues.empty", { ns: "starter" }, "No active tickets in this queue.")}
                </p>
              ) : null}
            </div>
          </div>
