...
import { BookOpenText, Search, ThumbsDown, ThumbsUp } from "lucide-react";
...
export function HelpArticlesPage() {
  ...
  return <div className="flex flex-col gap-6">
    <AnalyticsHeader title={translate("navigation.helpLibrary", { ns: "starter" }, "Help library")} description={translate("helpLibrary.intro", { ns: "starter" }, "Search structured support guidance, browse categories, and use article feedback to improve answer quality.")} actions={<label className="relative block w-full sm:w-80"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} className="pl-9" placeholder={translate("helpLibrary.searchPlaceholder", { ns: "starter" }, "Search titles and article content")} /></label>} />
