...
export function RequestersPage() {
  ...
  return <div className="flex flex-col gap-6">
    <AnalyticsHeader title={translate("requesters.title", { ns: "starter" }, "Requesters")} description={translate("requesters.description", { ns: "starter" }, "Understand who is asking for help, their company context, and the complete history behind each relationship.")} actions={<div className="relative w-full sm:w-72"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input className="pl-9" value={search} onChange={(event) => setSearch(event.target.value)} placeholder={translate("requesters.search", { ns: "starter" }, "Search requesters")} /></div>} />
