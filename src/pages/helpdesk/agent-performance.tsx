...
export function AgentPerformancePage() {
  ...
  const rows = [...ids].map((id) => {
    ...
    const csat = csatQuery.data?.find((item) => Number(item.assigneeId) === id)?.avg_csat ?? 0;
    return { id, name: agentDisplayName(agent, translate("tickets.assignee.unknown", { ns: "starter" }, "Unknown agent")), resolved: resolved?.n ?? 0, workload, avgResolution: Number(resolved?.avg_resolution ?? 0), csat: Number(csat), compliance: total ? ((total - breached) / total) * 100 : 100 };
  }).sort((a, b) => b.resolved - a.resolved);
  const totalResolved = rows.reduce((sum, row) => sum + row.resolved, 0);
  const active = rows.reduce((sum, row) => sum + row.workload, 0);
  const avgCsat = rows.length ? rows.reduce((sum, row) => sum + row.csat, 0) / rows.filter((row) => row.csat > 0).length : 0;
  const avgCompliance = rows.length ? rows.reduce((sum, row) => sum + row.compliance, 0) / rows.length : 0;
  ...
  return <div className="flex flex-col gap-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label={translate("performance.kpi.resolved", { ns: "starter" }, "Tickets resolved")} value={totalResolved} icon={<Tickets />} loading={loading} /><MetricCard label={translate("performance.kpi.workload", { ns: "starter" }, "Active workload")} value={active} icon={<Gauge />} loading={loading} /><MetricCard label={translate("performance.kpi.csat", { ns: "starter" }, "Team CSAT")} value={`${shortNumber(avgCsat)} / 5`} icon={<Award />} tone="success" loading={loading} /><MetricCard label={translate("performance.kpi.sla", { ns: "starter" }, "SLA compliance")} value={`${Math.round(avgCompliance)}%`} icon={<Clock3 />} loading={loading} /></div>
